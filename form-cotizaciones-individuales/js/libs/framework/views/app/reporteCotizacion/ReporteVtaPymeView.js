define(['jquery',
        ...
        'session',
        'text!/form-cotizaciones-individuales/js/libs/framework/templates/app/reporte/reporteVtaPyme.html',
        'text!/form-cotizaciones-individuales/js/libs/framework/templates/app/reporte/tablaVtaPymeReporte.html',
    ...
    function ($, _, Backbone, datatablesBT, Handlebars, bootstrap, ace, Encoding, SettingsModel, Util, LoginService,
              Session, ReporteVtaPyme, TablaReporte, ListPlanesTemplate, Combo, searchTemplate, ListSearchView,
              ProspectoService, CotizacionesService) {
        ...
        var contacto = null, cotizacion = null, obrasSociales = [], resultArray = null,
            ...
            reporteCotizacionView = Backbone.View.extend({
                ...
                searchSupervisor: function(){
                    ...
                    listSearchView.list("CN=GPS_REFERIDOS_SUPERVISOR,OU=CRM_PRUEBA,OU=APPLS,DC=SWM,DC=COM,DC=AR", this.$('#searchResultsContent'), this.$('#searchRefErrorContent'), callback);
                },
                ...
                buscar: function () {
                    ...
                    if (!this.validateRangoFechaCreacion()) {
                        $.gritter.add({
                            ...
                            text: 'Por favor las fechas no puede superar los 365 dias',
...
//                    searchFilter.Supervisor = $("#loginUsuario").val();
                    ...
                    var url = SettingsModel.get("cotizaciones_prospecto_rest_api_url") + 'reporte-venta-pymes?';
...
//                    if (searchFilter.Zona && searchFilter.Zona !== '') {
...
//                                fechaFin: moment(resultArray[i].fecha_fin_vigencia).format("DD/MM/YYYY"),
                                asesor: resultArray[i].registro_usuario,
                                usuario: resultArray[i].registro_usuario,
                                tramite: resultArray[i].nro_tramite,
                                estado: resultArray[i].estado,
                                motivo: resultArray[i].motivo,
                                razonSocial: resultArray[i].razon_social,
                                cuit: resultArray[i].cuit,
                                nombre: resultArray[i].nombre,
                                apellido: resultArray[i].apellido,
                                capAmba: resultArray[i].capitas_AMBA,
                                capInt: resultArray[i].capitas_interior,
                                email: resultArray[i].email,
                                tel: resultArray[i].Telefonos,
                                direccion: resultArray[i].Direccion,
                                localidad: resultArray[i].localidad,
                                provincia: resultArray[i].provincia,
                                observaciones: resultArray[i].Observacion
                            });
                    }
...
//                        {producto: "Producto Vendido"},
                        {motivo: "Motivo de Cierre sin Venta"},
                        {razonSocial: "Razon Social"},
                        {cuit: "Cuit"},
                        {nombre: "Nombre"},
                        {apellido: "Apellido"},
                        {capAmba: "Capitas AMBA"},
                        {capInt: "Capitas Interior"},
                        {tel: "Telefono"},
                        {email: "Email"},
                        {direccion: "Direccion"},
                        {localidad: "Localidad"},
                    ...
                    this.JSONToCSVConvertor(cotizacionesCSV, "Cotizaciones-Vta-Pymes-" + dateString, columnConf, function (row, index) {
                        if (row[index] == null) {
                            return "";
                        }
                        if (index === 'nroDoc') {
                ...
                render: function (target) {
                    ...
                    var compiledTemplate = Handlebars.compile(ReporteVtaPyme);
