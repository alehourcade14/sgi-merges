define ([
	...
	'text!libs/framework/templates/app/formAsignarSupervisorTemplate.html',
	...
	'text!/form-cotizaciones-individuales/aseguradoras.json'
			 ...
			 ListSearchView, formEditTemplate, taskDetailTemplate, taskNotFoundTemplate, actorNotFoundTemplate, searchTemplate,
			 ControladorArchivos, InboxService, LoginService, Session, FuseService, ConversorPresmedAfilmed, ProcessJson ,AseguradorasJson) {
	...
	var processDetail;
	var statusSave;
	var hasSubmotive, processNameJBPM, workcontext, allTasks;
	...
	var messagesSaveAndEnd = {
		'supervisor[descripcion]': {
			required: "El campo Supervisor es necesario."
		},
		'process[action]': {
			required: " "
		},
	...
	var newView = Backbone.View.extend ({
		...
		searchSupervisor: function(){
			...
			listSearchView.list("CN=AMB_OV_SUPERVISOR,OU=CRM_PRUEBA,OU=APPLS,DC=SWM,DC=COM,DC=AR", this.$('#searchResultsContent'), this.$('#searchRefErrorContent'), callback, true);
		},
		...
		saveAndEndTask: function () {
			...
			if(!form.valid() || usuarioNotSelected){
				$.gritter.add({
					title: '<i class="icon-exclamation-sign bigger-120"></i>&nbsp;Atención',
					text: 'Por favor ingrese los datos necesarios',
					class_name: 'gritter-warning'
